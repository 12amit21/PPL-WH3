...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> =
    (params) => params.length === 1 ?
        bind(parseL4CExp(params[0]), (c : CExp) => isVarRef(c) ? makeOk(makeTraceExp(c)) :
            makeFailure('parsed parameter is not varRef')) : makeFailure("params length is bigger than 1");
